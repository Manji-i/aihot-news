...
#    - LARK_CHANNEL_HOME / LARK_CHANNEL_PROFILE 等（如使用 lark-channel）
nano config.sh
...
| `LARK_CHANNEL` | 使用 lark-channel 时必填 | 设为 `1` 启用 bridge 模式 |
| `LARK_CHANNEL_HOME` | 同上 | lark-channel 安装目录 |
| `LARK_CHANNEL_PROFILE` | 同上 | profile 名称 |
| `LARKSUITE_CLI_CONFIG_DIR` | 同上 | lark-cli 配置目录 |
| `API_BASE` | 否 | API 地址，默认 `https://aihot.virxact.com` |
| `MAX_PER_MESSAGE` | 否 | 每条消息最多几条新闻，默认 3 |
| `PAGE_SIZE` | 否 | 每页拉取条数，默认 50 |

> **不用 lark-channel？** 如果你只用独立 lark-cli（非 bridge 模式），把 `config.sh` 中 `LARK_CHANNEL` 等 4 行注释掉即可，脚本会用你本机默认的 lark-cli 配置。
