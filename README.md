...
每 5 分钟轮询 [AI Hot](https://aihot.virxact.com) 精选 AI 新闻，增量推送到飞书/Lark 群聊。脚本会回看一段时间窗口并用条目 ID 去重，避免 AI Hot 后补精选时漏推。
...
| `LOOKBACK_HOURS` | 否 | 回看窗口小时数，默认 12；用于补抓后补精选 |
| `MAX_SENT_IDS` | 否 | 保留多少个已发送条目 ID 用于去重，默认 1000 |
...
├── state.json           # 游标 + 已发送 ID 去重状态（自动生成，不提交）
...
运行日志写入 `poll.log`（脚本所在目录）。需要无发送验证时，可以运行：

```bash
AIHOT_DRY_RUN=1 ./cron-wrapper.sh
```

建议配置 logrotate 或定期清理：
...
| 精选页有但群里没推 | AI Hot 可能后补较早 `publishedAt` 的精选条目，旧版单游标会跳过 | 升级到新版；新版默认回看 12 小时并用 `sent_item_ids` 去重 |
